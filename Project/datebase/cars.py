...
def SelectTable(dbpath):
    try:
        ...
        sqlite_selection_query = "SELECT * FROM cars;"
...
def selectFromnames(name,dbpath):
    try:
        ...
        sqlite_selection_query = "SELECT * FROM cars WHERE name=?;"
...
def selectFromids(id,dbpath):
    try:
        sqlite_connection = sqlite3.connect(dbpath)
        cursor = sqlite_connection.cursor()
        print('Соединение с базой данных прошло успешно.')

        sqlite_selection_query = "SELECT * FROM cars WHERE id=?;"
...
def addNewcar(record: list,dbpath):
    ...
    try:
        ...
        insert_query = '''INSERT INTO cars (name)
                          VALUES (?);'''
...
def deleteRecord(id,dbpath):
    try:
        ...
        sqlite_delete_query = "DELETE FROM cars WHERE id=? ;"
